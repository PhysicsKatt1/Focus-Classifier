...
train_and_val_outputs = r'/RegressionData_30kV_0.09nA_train_defocusStig'
    ...
    for images, n in zip(os.listdir(train_and_val_inputs + '/' + subfolders),
        ...
        try:
            ...
            if 'Focus_data_30000.0V_0.09nA__' in images:
                ...
                if float(defocus) != 0: # float(stigx) == 0.0 and float(stigy) == 0.0 and 
                    im_count += 1
                    ...
                    labels.append({'Image': str(im_count), 'Voltage': 30000.0, 'Current': 0.09, 
                                   'Defocus': defocus, 'StigX': stigx, 'StigY': stigy})
...
##### uncomment to add more data to an existing folder #####
